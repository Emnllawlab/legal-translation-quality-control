...
这些内容是供核查的精简触发词，不是词典，也不界定高风险术语的完整范围。每一行只记录一种已知失误模式，提示需要识别的语境或法律效果。未列入锚点不代表风险较低；仍应按 Tier 1 标准动态判断。具体措辞应结合源法、权威平行文本、目标法域和项目术语表确定，不得机械照抄锚点。
...
| accrued right | 权利已经产生或既得，不等同于债务已经到期或可立即请求履行 | 语境表达的是 `accrued/vested`，还是 `due/matured`？不要把 accrued right 译成“已到期权利”。 |
| rescission | 普通法中使合同回溯消灭的机制，或成文法上的取消；现行中国《民法典》合同编直接英译中可作为“解除”的默认项目译法 | 原文使用的是普通法 rescission、中国法合同解除，还是其他法律效果？是否需要核对具体法源和平行文本？ |
